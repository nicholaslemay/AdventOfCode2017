...
def as_array_of_integers(row):
    return map(int, row.split("\t"))

def parsed(text):
    rows = text.split("\n")
    non_empty_rows = filter(lambda r: r != '', rows)
    return map(as_array_of_integers, non_empty_rows)

def dividable_numbers_division(numbers):
    for dividend in numbers:
        for divisor in numbers:
            if(dividend != divisor and (dividend % divisor == 0)):
                return dividend / divisor       

def checksum_for(items):
    divisions = map(dividable_numbers_division, items)
    return sum(divisions)

class TestChecksum(unittest.TestCase):

    def test_as_array_of_integers(self):
        sample_text = """
5	1	9	5
7	5	3
2	4	6	8
"""
        self.assertEquals(parsed(sample_text), [[5,1,9,5],[7,5,3],[2,4,6,8]])
     
    def test_dividable_numbers_division(self):
        self.assertEqual(dividable_numbers_division([5, 9, 2, 8]), 8 / 2)
        self.assertEqual(dividable_numbers_division([9, 4, 7, 3]), 9 / 3)
        self.assertEqual(dividable_numbers_division([3, 8, 6, 5]), 6 / 3)
...
"""
        result = checksum_for(parsed(sample_text))
